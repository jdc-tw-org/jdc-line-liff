 ...
 *
 * ══ 為何是一支 asset，不是每一頁各寫一段 ═══════════════════════════════
 *
 * 現象（2026-09-16 實測）：`git grep -c "me\.html"` 掃票上那六個功能內頁，
 * 五頁回 0、`line-messages.html` 回 1——而那 1 是一句註解，不是入口。
 * ⇒ 六頁都沒有回得去的路，使用者只能按瀏覽器返回鍵或重打網址。
 *
 * 直覺的修法是「每一頁各加一個連結」。那正是這個 repo 反覆踩的形狀：
 ...
 *
 * 🔴 **「哪幾頁該掛」的權威來源不是票，是後端 `roles.js` 的 DISPATCH 表。**
 *    （2026-09-17 第二輪補上 `authz.html`）
 *    票 #104 的內文手寫了六頁，而 `authz.html` 在票開出來之後才上線
 *    ——第一輪照著那六頁做完、全套測試全綠，**站上會是 7 頁裡 6 頁有返回入口，
 *    第 7 頁靜默沒有**。抓到它的不是任何一條測試，是有人去讀了權威來源。
 *    ⇒ `tests/back-to-me.test.js` 現在**從 `tests/fixtures/action-roles.json`
 *      的 `dispatchPages` 讀那份清單**，不再手抄。分流表新增一頁時它會紅，
 *      而修法就是給那一頁也加上上面那一行。
 *
 ...
 *    而且這幾頁裡連一個 `<form>` 都沒有（實測剝註解後 0 命中，`authz.html` 含在內）。
 *    這是「做不到」，不是「不該做」。
 ...
 *
 * 這幾頁頂端實際上的三種形狀：
 *   · `board`／`stats`／`hr-stats`／`attend`／`authz` —— 普通流，body 自己就是置中的內容欄
 ...
 *   （其餘六頁的 body 本身就是內容欄，所以退路對它們是正解，不是降級）。
 *
 ...
 *    身分確認完、閘拿掉之後，這排字就在內容欄左上角，與其他六頁同一個位置。
 *
 * ══ 「淡淡的一排字」怎麼落地 ═══════════════════════════════════════════
 *
 ...
 *    鍵盤使用者會完全不知道自己走到了哪裡。
 *
 * ⚠️ 每一個 CSS 變數都帶 fallback：七頁裡只有 `attend.html` 與 `authz.html` 載入
 *    `assets/ui.css`，其餘五頁各自帶著自己那份 `:root`（見 `ui.css` 檔頭的退場條件）。
 *    沒有 fallback 的話那五頁會拿到瀏覽器預設的藍色連結——**而且不會報錯**。
 *    ⚠️ **`board.html` 兩種都沒有**（不載 ui.css、也沒有自己的 `:root`）
 *       ⇒ 它是唯一一頁「fallback 被改壞會當場看得見」的頁。
 * ⚠️ 顏色用 `--ink3`（色票裡最弱的那一階，`#9a9a96`）。**再淡就撐不住對比**，
 *    而票上明寫「桌面寬度下每一頁都看得到」——淡到量不出來就是失敗。
 */
(function () {
  'use strict';
  ...
  var TARGET = 'me.html';
  /** 連結文字＝accessible name（檔頭④）。所有內頁共用這一個字串。 */
  var LABEL = '‹ 回功能選單';

  /**
   * 「這一頁的內容欄是誰」。**唯一一張表**，只有頂端結構特殊的頁需要列。
   * 沒列到的頁退回 `document.body`——那幾頁的 body 本身就是置中的內容欄。
   *
  ...
  var CSS = '#backtome{margin:0 0 4px;padding:0;line-height:1.5}'
    // ⚠️ **只有上內距，左右一律 0。** 實測（1280 寬）各頁的 body 內容盒左緣分別在
    //    board 284／stats 304／hr-stats 276／attend 304／line 304／authz 416 px
    //    ——body 自己已經有左右內距了，這裡再加 16px 的話，
