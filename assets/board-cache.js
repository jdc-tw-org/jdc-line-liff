 ...
 *
 * 🔴 **這一段的理由在 2026-09-18 換過一次，舊的那個已經是假的。**
 *    舊文寫的是「因為還有不帶 `reason` 的生產者——`Code.js` handler 層那 **75 處**，
 *    這條退路就是為它們留的」。⬛ 後端 `#163` 把 handler 層那 96 處**全部補上代號**
 *    之後，那句話就不成立了：那 75 處今天**每一處都帶 `reason`**，而前端拿到代號就
 *    不看文字 ⇒ **這條退路對它們一格都沒有作用。**
 *    ⚠️ 它壞在「理由還在」那一邊（`feedback_comment_enshrines_a_bug` 第四形態）：
 *       句子讀起來仍然像個成立的理由，而它指名的那個前提兩張票之前就被改掉了。
 *       ⇒ 讀到這種「因為 X，所以留著」時，該去驗的是 X，不是尊重那道禁令。
 *
 * ⇒ **今天這條退路只接得住三個東西**：後端守門 `allow === null` 的三格
 *   （action 名不在白名單裡——打錯、已拆、或前端版本落後於後端）。它們**刻意不帶代號**，
 *   後端 `#140` 已經替它們掛上具名豁免標記。**除此之外沒有第四個生產者**——
 *   ⬛ 逐一列舉後端那兩個檔裡「那一句話」的每一個出口：handler 層全部帶
 *   `reason`（另有全稱命題守門釘著），守門層其餘各格也都帶，只有這三格不帶。
 *
 * 🔴 **而這三格今天會被判成 `revoked`** ⇒ 清光他裝置上那份加密快取 ＋ 蓋整頁覆蓋層
 *    ⇒ **權限好好的人打到一個不存在的 action 名，那一頁對他當場報廢。**
 *    ⬛ 2026-09-18 **真瀏覽器**實跑（localhost，不是單元測試）：3 把加密快取鍵 → 0、
 *      覆蓋層蓋滿整頁（底下已經畫好的 153 個人名全部看不見）、`isRevoked()` 轉 true。
 *    ⚠️ **擁有者 2026-09-18 在知道這個代價之後拍板保留這個行為**（只加標記、不改行為）。
 *      ⇒ **所以這條退路今天不拆**——不是因為還有生產者需要它（沒有了），
 *        是因為拆掉會改變那個剛拍板的決定。拆掉之後這三格會變成 `ok`（⬛ 實測，見下）。
 *
 * ══ ⏳ 退場條件（量法已經搬走，這裡只指過去）═══════════════════════════
 *
 * 🔴 **量法不再抄在這裡，這是刻意的。** 2026-09-17 這個檔頭裡曾經有**兩把尺的副本**，
 *    而它們量的是**另一個 repo（私有）的原始碼** ⇒ 公開 repo 的一句註解量著私有 repo
 *    的碼，中間**沒有任何強制點**：後端長出一個新的、不帶代號的權限拒絕出口，
 *    不會有任何東西變紅。（`feedback_shell_in_docs_untestable`。）
 *    ⬛ **而它真的爛掉了**：後端 `#140` 把述詞收緊之後，照這裡那份舊副本跑會回 **`8`**，
 *      新尺回 **`0`**。⇒ 退場條件照字面讀會說「還沒到」，而真實答案是「到了」。
 *      ⚠️ 它壞在「**看起來還沒到**」那一邊——最容易被讀成「那就再等等」，
 *        然後永遠等下去。**這正是這個檔頭自己警告過的形狀。**
 *
 * ⇒ **尺的權威在後端**：`jdc-line-gas` 的 `line-platform/tools/deny-reason-ruler.js`，
 *   強制點是它旁邊的 `tests/deny-reason-ruler.test.js`（後端新增一個不帶代號的
 *   權限拒絕出口，那支就會紅）。在該 repo 的根目錄跑：
 *
 *     node line-platform/tools/deny-reason-ruler.js
 *
 *   **退場條件＝第一個數字（`不帶代號`）回 `0`。**
 *   ⬛ 對照組就是它一起印的另外幾個數字：尺壞掉時（路徑錯、剝註解把整份吃掉、
 *     正規式沒命中）**全部一起變 0** ⇒ `不帶代號=0` 只有在其他幾個仍是大數字時才算數。
 *
 * ⬛ **2026-09-18 實跑**（gas `d4374a5` 與 `e2ec970` 兩顆逐字相同）：
 *     `不帶代號=0  具名豁免=3  帶代號=111  不是權限拒絕=319  全部拒絕出口=433`
 *   ⚠️ **數字跟著 SHA 走，引用前先重跑，不要照抄。**
 *
 * 🔴 **第一個數字已經是 0 了，但退路今天不拆。** 退場條件是「**可以**拆」的必要條件，
 *    不是「**該**拆」的充分條件——擋在前面的是上面那個擁有者決定。見 `#64`。
 *
 * ══ ⬛ 拆掉會發生什麼（2026-09-18 實測，不是推論）═══════════════════════
 *
 * 拿**後端真的送出來的信封**逐一餵 `cacheVerdict`，現行 vs. 只拆掉那一行：
 *   · 守門三格（`{ok:false, msg:<那一句>}`、不帶代號）：`revoked` → **`ok`**（三格全變）
 *   · 契約表 17 列（後端真的跑過守門吐出來的信封）：**0 列改變**
 *   ⬛ 對照組：帶 `role_unresolved` 的那一句仍是 `revoked`、離線仍是 `offline`
 *     ⇒ 這個比較**分得出**差異，不是恆等。
 * ⬛ 真瀏覽器覆驗：拆掉之後，守門三格那個信封**什麼都不做**（3 把快取仍在、無覆蓋層），
 *   而真正的撤權（帶 `role_unresolved`）**照樣**清光快取＋蓋覆蓋層。
 * ⇒ **拆掉不會少遮蔽任何一條真的撤權路徑**，它只拿掉那三格的誤判。
 *   ⚠️ 但那正是擁有者拍板要保留的行為 ⇒ **要拆之前回去問，不要自己決定。**
 *
 * 🔴 **這條退場條件有兩個端點，要一起刪。** 另一端在後端：那一句話逐字交給前端的
 *    那一格匯出（就是本檔測試讀的 `gateContract.legacyDenyMsg` 的來源）。
 *    它存在的唯一理由就是餵這條退路 ⇒ 退路刪掉，它也沒有用途了。
 *    ⚠️ **但後端那把尺不一定同批刪。** 退路一拆，它就變成「不准再長出不帶代號的
 *    權限拒絕出口」的**唯一**守門——拆掉之後這種出口在前端一律回 `ok`＝什麼都不做，
 *    而**什麼都不做跟正常長得一模一樣** ⇒ 它**可能更需要留著**。這是擁有者的決定。
 *
...
function cacheVerdict(resp) {
  ...
  // 🔴 **今天這一行只接得住後端守門 `allow === null` 的三格**（action 名不認得）。
  //    handler 層那些**已經全部帶代號了**（後端 `#163`）⇒ 它們走不到這一行。
  //    ⚠️ 不要靠放寬前綴去接別的——放寬前綴＝把「要不要清光他的快取」交給文案決定。
  // ⏳ 退場條件的第一個數字**已經回 0 了**（量法在檔頭，權威在後端那把尺）。
  //    🔴 **但這一行今天仍然不拆**：拆掉會讓那三格從 `revoked` 變 `ok`，
  //    而擁有者 2026-09-18 知情拍板保留那個行為。**要拆先回去問**（`#64`）。
