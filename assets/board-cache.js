 ...
 *
 *   ① `jdc-line-gas` `Code.js` handler 層、msg 剛好是那一句的（⬛ 2026-09-17 量於 `main`
 *      `1d5784e`：**75 處**）⇒ 前綴命中 ⇒ **這條退路就是為它們留的**。
 *      那一層失敗的意思是「守門放行了、但這支功能要看板身分」，與守門層不是同一件事。
 *   ② 同一層、同樣不帶代號，但 msg 換了寫法的（⬛ 同一次量到另外兩種寫法共 **21 處**，
 *      另有一處寫成「…的檢視權限。」）；以及後端守門 `allow === null` 那一格（action 不認得，
 *      **刻意不帶代號**——帶了就會對外洩漏哪些 action 存在）。
 ...
 *      `不帶代號=105 帶代號=23 全部拒絕出口=433`。**讀到這段先重跑，不要照抄。**
