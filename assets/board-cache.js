 ...
 *
 * ══ 🪦 那條 msg 前綴退路已經拆掉了（2026-09-18，liff `#74`，擁有者拍板）═════
 *
 * 🔴 **下面只吃 `reason`、對「沒有代號」一律回 `ok`——那是刻意的，不是漏寫。**
 *    ⚠️ 這一段不是歷史散文，是為了讓下一個人**不必再調查一次**：他會看到後端守門
 *       那三格回 `ok`，如果這裡什麼都沒留，他只能猜那是缺陷。
 *
 * **它曾經是什麼**：2026-09-17 改吃代號時，這裡留了一行「拿舊那句拒絕文案做前綴比對
 * ⇒ 判 `revoked`」的退路，給當時還不帶 `reason` 的生產者（後端 handler 層那一批）用。
 * ⬛ 後端 `#163` 把 handler 層**全部補上代號**之後，那個理由就空了——前端拿到代號就
 * 不看文字 ⇒ 那條退路對它們一格都沒有作用。⚠️ 它壞在「理由還在」那一邊
 * （`feedback_comment_enshrines_a_bug` 第四形態）：句子讀起來仍像個成立的理由，
 * 而它指名的前提兩張票之前就被改掉了。**讀到「因為 X 所以留著」時該驗的是 X。**
 *
 * **它最後接得住的只有三個東西**：後端守門 `allow === null` 那三格（action 名不在
 * 白名單裡——打錯、已拆、或前端版本落後於後端）。它們**刻意不帶代號**，後端 `#140`
 * 已替它們掛上具名豁免標記。⇒ 現行行為是「權限好好的人打到一個不存在的 action 名，
 * 那一頁對他當場報廢」（清光加密快取＋蓋整頁覆蓋層）。**拆掉的就是這個誤判。**
 *
 * ── ⏳ 退場條件（已成立、已執行，這一段是它的墓碑）──────────────────
 *
 * 🔴 **量法從來不抄在這裡，這是刻意的**：公開 repo 的一句註解量不到私有 repo 的碼，
 *    中間沒有任何強制點。⬛ 而抄過的那一份**真的爛掉了**——後端 `#140` 收緊述詞之後，
 *    照舊副本跑回 `8`、新尺回 `0`：照字面讀會說「還沒到」，真實答案是「到了」。
 *    ⚠️ 它壞在「**看起來還沒到**」那一邊，最容易被讀成「那就再等等」然後永遠等下去。
 *
 * ⇒ **尺的權威在後端**：`jdc-line-gas` 的 `line-platform/tools/deny-reason-ruler.js`
 *   （強制點是它旁邊的測試）。在該 repo 根目錄跑 `node <上面那支>`，
 *   **退場條件＝第一個數字（`不帶代號`）回 `0`**。
 *   ⬛ 對照組就是它一起印的另外幾個數字：尺壞掉時（路徑錯、剝註解把整份吃掉、
 *     正規式沒命中）**全部一起變 0** ⇒ 第一個數字只有在其他幾個仍是大數字時才算數。
 *
 * ⬛ **拆的那天實跑**（後端 main `e2ec970`，本票獨立重跑、與 `#64` 逐字相同）：
 *     `不帶代號=0  具名豁免=3  帶代號=111  不是權限拒絕=319  全部拒絕出口=433`
 *     三格具名豁免的鍵：`gate-action-unknown` / `gate-action-by-line-unknown` /
 *     `gate-line-identity-unknown`。⚠️ **數字跟著 SHA 走，引用前先重跑，不要照抄。**
 *
 * ── ⬛ 拆的那天量到什麼（實測，不是推論）───────────────────────────
 *
 * 拿**後端真的送出來的信封**逐一餵 `cacheVerdict`，拆前 vs. 拆後：
 *   · 守門那三格（`{ok:false, msg:<舊那句>}`、不帶代號）：`revoked` → **`ok`**（三格全變）
 *   · 契約表 17 列（後端真的跑過守門吐出來的信封）：**0 列改變**
 *   ⬛ 對照組：帶 `role_unresolved` 的那一句仍是 `revoked`、離線仍是 `offline`
 *     ⇒ 這個比較**分得出**差異，不是恆等。
 * ⬛ **真瀏覽器**（localhost、真 Chrome，不是單元測試）：先讓快取真的長出 **7 把**，
 *   再餵那三格的信封——**拆前 `0 把 ＋ 整頁覆蓋層`，拆後 `7 把仍在 ＋ 無覆蓋層`**；
 *   而**真正的撤權**（帶 `role_unresolved`）**拆前拆後都是 `0 把 ＋ 整頁覆蓋層`**。
 *   ⬛ 另一組對照：不相干的錯誤（`找不到員工名冊`）拆前拆後都是「7 把仍在、無覆蓋層」。
 * ⇒ **這次沒有少遮蔽任何一條真的撤權路徑**，拿掉的只有那三格的誤判。
 *
 * ── ⚠️ 拆掉之後，失敗換到另一邊了 ──────────────────────────────
 *
 * 🔴 不帶代號的權限拒絕，從「整頁報廢」變成「**畫面停在快取、不再更新**」——
 *    ⇒ 後端那把尺（「不准再長出不帶代號的權限拒絕出口」）從此是**唯一**的守門，
 *    **比拆之前更需要留著**，不要順手清掉。
 *
 * ⬛ **但它不是完全沒有提示**（真瀏覽器實測，`board.html` 新人報到分頁）：畫面停在
 *    快取之後，`settleRefresh` 會在區塊上方掛出琥珀提示，**逐字帶著後端那一句**
 *    （`⚠️ 這裡顯示的是 <時間> 的資料，目前無法更新：<後端原文>`）。
 ...
 *    拿不到 hub 的網址。⇒ 後端 `roles.js` 那句「同一句話有第二個生產者」已經過期。
 *
...
function cacheVerdict(resp) {
  ...
  // 🪦 **沒有代號的一律 `ok`——刻意的，不是漏寫。** 這裡曾經有一行拿舊那句拒絕文案
  //    做前綴比對、判 `revoked` 的退路，2026-09-18 拆掉（liff `#74`，擁有者拍板）。
  //    為何、退場條件怎麼成立的、拆那天量到什麼，見檔頭 🪦 那一段。
  //    ⚠️ **要放回去之前先讀那一段**：`tests/board-cache.test.js` 有一條會紅，
  //       而且放回去就是把「打到一個不存在的 action 名 ⇒ 整頁報廢」也一起放回去。
  return 'ok';
}
 ...
 *
 * ══════════════════════════════════════════════════════════════════════════
 * 🔴 你正在寫一個新的 SWR 區塊嗎？先讀這一段（`#82`，2026-09-18 擁有者拍板）
 * ══════════════════════════════════════════════════════════════════════════
 *
 * 🔴 **這支的呼叫端是逐處手寫的。沒有任何東西要求「每個 SWR 區塊都有一格」。**
 *    ⚠️ 不要把這段話讀成「有守門盯著」——**沒有**。這是一個**已知缺口**，
 *    寫在這裡是因為下一個寫 SWR 區塊的人一定會打開這支函式，而那是目前
 *    唯一能把成因放到他眼前的地方。**它不是強制點。**
 *
 * ⬛ **已經漏過一次（量出來的，不是假設的）**：`index.html` 的 `loadLottery`
 *    ——先讀 `sessionStorage` 畫獎金卡、再打 `getMyLottery` 校正，而那個校正的
 *    **兩條失敗分支一條 `return`、一條只寫 `?diag=1` 的飛行紀錄** ⇒ 伺服器壞掉時
 *    畫面停在上一次的中獎金額，一個字都不說。修於 `#82`。
 *    ⚠️ 它從來沒被任何母體看見過：先前數的是「本檔的呼叫點」（7 個檔 23 處），
 *    而**一個從沒用過本支函式的區塊，結構上永遠不會出現在那張表上**。
 *
 * ── 🔘 為什麼沒有立守門（三案都量過了，不是沒想過）───────────────────────
 *
 * **乙｜掃原始碼的測試守門**：實測**綠不可信**。把提示補在 `fetchLottery` 的 `.catch`
 *   （＝**最自然的修法**）那顆突變**活了下來**：尺照樣紅。成因是尺的範圍是
 *   「讀取點所在的那支函式」，而提示天然屬於「網路回應所在的那支函式」，**兩者常常不同支**。
 *   ⇒ 上線後的樣子是「有人照最自然的方式修好了，燈還是紅的」⇒ 他會學會無視那一格，
 *   比沒有守門更糟。附帶：recall **36/47**（47 個讀取點有 11 個寫成 `var f=function(){}`／
 *   箭頭／IIFE，尺什麼都答不出來），3 個命中裡 **2 個是假陽性**（`admin.html`／`staff.html`
 *   那種「命中快取就完全不打 API」的頁），得另寫一份會腐爛的豁免清單。
 *
 * **丙｜改成集中式（讓本支自己掛提示，呼叫端無從漏掉）**：它動的正是 **2026-08-20 誤發事故**
 *   那條共用路徑（見上面那句「提示插在區塊上方的兄弟節點，不動區塊本身」）。
 *   要動 23 個呼叫點，而「有沒有保住那個性質」**在單元測試裡看不出來**。
 *
 * ── 🔴 什麼情況該回頭重新考慮（可證偽，不是日期。任一成立就回來）──────────
 *
 *  ① **再出現第 2 個**「畫面上有快取、刷新失敗卻一個字都不說」的缺口。
 *     `#82` 是第 1 個。第 2 個成立時，「逐處手寫」就已經**被證明會復發**，
 *     這段註解的補償效果同時被證偽（第 2 個的作者就是讀了這段還是漏了）。
 *     ⚠️ 怎麼算：**修掉它的那張票**算一次，不是靠印象。
 *  ② **有人拿出一把殺得掉那顆突變的尺**——把提示補在「網路回應所在的那支函式」時它要判綠。
 *     那是否決乙的**唯一**理由；這一格被證偽，乙就重新可選。（要驗只要跑那一顆突變。）
 *  ③ **`index.html` 因為別的理由接上了 `assets/board-cache.js`**。丙的代價裡有一項是
 *     「`index.html` 用的是另一套儲存與畫法，要一起收就得把它也接上來、範圍再擴一圈」
 *     ——那項一旦消失，集中式要重新評估。（要驗只要 `grep board-cache index.html`。）
 */
