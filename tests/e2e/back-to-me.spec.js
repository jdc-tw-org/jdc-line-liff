...
/**
 * 「等不到那個節點」到底是**受測物壞了**還是**量具壞了**——逐格排除，一句結論配一格證據。
 *
 * 🔴 **每一句結論都要有一格證據撐著。**「訊息要指得出來」不是「訊息要很長」。
 *    ⚠️ 2026-09-17 驗證軌證偽過本函式的前一版：它只看**網址與標題**就下結論，
 *    於是遇到「回應在 `</title>` 之後就斷掉」時——**標題對、網址也對，而頁面根本沒載完**
 *    ——它會說「那排字真的不見了（載入行被繞過）」。**那是把量具的故障歸因到受測物身上。**
 *    分得出來的那一格當時**已經印在現場 dump 裡了**（`body第一個子元素: null`），
 *    只是結論沒有去吃它。⇒ 現在吃了。
 *
 * ⚠️ 排除的順序要由「離受測物最遠」往回走，最後才輪到受測物；
 *    只要還有任何一格指向量具，就不可以說是受測物的問題。
 */
function 判成因(現場, 檔名) {
  if (現場.取不到現場) {
    return '🔧 量具：連現場都取不到（' + 現場.取不到現場 + '）'
      + '⇒ 頁面已經不在了（被導走或關掉），這一輪什麼都沒量到。';
  }
  if (String(現場.網址 || '').indexOf('/' + 檔名) < 0) {
    return '🔧 量具：**頁面被導走了**（現在在 ' + 現場.網址 + '）'
      + '⇒ 量到的是另一份文件，不是受測頁。';
  }
  if (!現場.文件標題) {
    return '🔧 量具：**文件標題是空的** ⇒ 這一頁根本沒回來（404／伺服器沒給內容）。';
  }
  // 🔴 這一格就是前一版漏掉的那一格。標題是在 `<head>` 裡就定下來的，
  //    所以**回應只要撐到 `</title>` 就會有標題**——標題對不代表頁面載完了。
  if (現場.body第一個子元素 === null) {
    return '🔧 量具：標題與網址都對，**但 body 裡一個元素都沒有** '
      + '⇒ **回應中途就斷掉了、頁面沒載完**（標題在 `<head>` 就定了，'
      + '所以「標題對」證明不了頁面完整）。先修儀器，不要當成受測物的問題。';
  }
  return '✅ 受測物：頁面確實載起來了（網址對、標題「' + 現場.文件標題
    + '」、body 首子元素 ' + 現場.body第一個子元素 + '），而那個節點不存在 '
    + '⇒ **那排字真的不見了**（載入行被繞過：整行被刪、被註解、被未閉合的 `<!--` 吃掉、'
    + '被 `<template>` 包住、改成不可執行的 type、或只在字串裡留下完整標籤片段'
    + '——這幾種在原始碼字串比對下全都看不出來）。';
}
...
async function 開頁(page, 檔名, opt) {
  ...
  if (!掛上了) {
    ...
    expect(掛上了, '等不到返回入口的節點（' + 連結 + '）。'
      + 判成因(現場, 檔名) + ' 現場：' + JSON.stringify(現場)).toBe(true);
  }
