 ...
 * ══ ⚠️ 跨工作樹撞 port ═════════════════════════════════════════════════
 *
 * 4173 是這個 repo 各工作樹**共用**的預設號碼，而同一時間常有好幾棵樹在跑。
 * `reuseExistingServer` 沒開的話撞 port 會死得很明顯，開了則會**靜默去測別人的樹**。
 * ⇒ 自己這一棵要跑之前先挑一個沒人在用的：`E2E_PORT=4291 npx playwright test`。
 *   `baseURL` 與 `webServer.url` 兩格都從 `tests/e2e/port.js` 取，會一起跟著走。
 *   本檔第一條測試另外用 md5 證明**伺服器送出來的 `me.html` 就是磁碟上這一份**。
 *
 * ⬛ **更正（`2968abb` 之前，下面這段原文是對的）**：原文寫的是
 *    「本檔配一份自己的 playwright 設定（兩格都要換，**只設 `E2E_PORT` 是不夠的**）」。
 *    那在 `playwright.config.js` 還把 port 寫死成 4173 的時候成立——設了環境變數，
 *    伺服器會換 port、瀏覽器卻還是去敲 4173。`2968abb` 把兩格都接到 `tests/e2e/port.js`
 *    之後**設 `E2E_PORT` 就夠了**；那句話再留著，會把下一個人導回
 *    「在 repo 外面自己另做一份設定」——那正是它本來要避免的麻煩。
 *
 * ⚠️ 原文指名的那份「本檔自己的 playwright 設定」，**我實際查過、沒有找到**：
 *    掃過 27 棵工作樹的工作目錄，以及各遠端分支的檔案清單，`me-relogin` 那條分支上
 *    就只有 `playwright.config.js`。掃到的三份非標準設定（`pw.verify.config.js`／
 *    `pw.v111.config.js`／`pw-w119.config.js`）都是別軌為了繞 port 臨時放的、未進版控，
 *    不是這一支要的東西。⬛ 這條尺抓得到非標準設定——它就是這樣找到那三份的。
 */
