...
const 碼 = {
  戊: 'JDC-AAAAAA', 甲: 'JDC-BBBBBB', 乙: 'JDC-CCCCCC',
  丙: 'JDC-DDDDDD', 丁: 'JDC-EEEEEE', 己: 'JDC-FFFFFF',
};
const 單位甲 = '測試單位甲', 單位乙 = '測試單位乙';

/**
 * 名冊順序照後端（按內部碼遞增）：
 *   戊(空) 甲(甲單位) 乙(乙單位) 丙(甲單位) 丁(乙單位) 己(全空白)
 * ⇒ 分組後是 甲丙 ／ 乙丁 ／ 戊己，**與名冊順序不同** ⇒ 排序真的發生了。
 *
 * 🔴 **戊（沒填部門的那個）刻意排在名冊最前面**（⒟）。
 *    ⬛ 這一格是突變逼出來的：沒填的人全排在名冊後面時，「未填那一組墊底」
 *    與「照出現順序」畫出來**一模一樣** ⇒ 把墊底那兩行拿掉，這一檔會全綠
...
test('⬛ 鑑別力：分組真的搬動了人——名冊順序與下拉順序**不同**', async ({ page }) => {
  ...
  expect(名冊序.join(','),
    ...
    .toBe([碼.戊, 碼.甲, 碼.乙, 碼.丙, 碼.丁, 碼.己].join(','));
  expect(序.join(','),
