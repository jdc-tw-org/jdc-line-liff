...
/* ══ 🪦 那條 msg 前綴退路的墓碑：**這裡是它的家** ═══════════════════════
 *
 * 這一格曾經是**退場條件**的家（2026-09-17，gas #140）：檔頭是給**讀**的人看的，
 * 但真正會刪掉那條退路的人，是被下面那條測試**擋紅**的人。退場條件要寫在他被迫看見
 * 的那一格才有觸發點；寫在機制自己的標頭裡不夠——他可以一次都沒打開過那個檔頭。
 *
 * ⏳ **退場條件在 2026-09-18 成立，擁有者拍板拆掉，退路已經不在了**（liff #74）。
 *    ⇒ 這一格的任務跟著換了一邊：以前它擋的是「不要隨便拆」，現在它擋的是
 *    **「不要隨手放回去」**。下面那條斷言就是那道紅字。
 *
 * 🔴 **判準不是「那句話還在不在」，是「那些出口帶不帶 `reason`」。**（留著這句是因為
 *    它當初就是量錯的病根。）原本的量法數的是後端字面文案的行數，兩個洞、方向相反：
 *      · **假放行**：同一層另外兩種寫法一個都數不到；光是**改寫文案**就能讓它回 0
 *        ⇒ 一個代號都沒補，退場條件就宣告成立。
 *      · **零鑑別力**：守門那幾格用的是常數名、不是字面量 ⇒ 它回的 `roles.js=1` 是常數
 *        宣告本身，那幾格補沒補代號都不會動。
 *
 * ══ 🔴 2026-09-18：釘法換過一次（gas `#203`）══════════════════════════
 *
 * 舊釘法是「拿 `gateContract.legacyDenyMsg` 那一句餵進去 ⇒ 必須回 `ok`」。
 * ⚠️ **那一格匯出的唯一消費者就是這一條測試**（退路拆掉之後，`board-cache.js` 不再讀它）
 *    ⇒ 後端要拆那一格匯出，會讓這一條因為**副本缺欄位**而紅——而那個紅讀起來像
 *    「副本沒重產」，不像「後端故意拿掉了」。
 *
 * ⇒ 改釘一個**不需要那一格也成立、而且涵蓋範圍只增不減**的全稱命題：
 *   🔴 **契約表裡後端真的送出來的每一句拒絕文案，抽掉 `reason` 之後都必須回 `ok`。**
 *
 * ⬛ 為什麼這是「只增不減」：那條退路認的那一句（`legacyDenyMsg`）**本來就在契約表裡**
 *   ——`revoked_token_path` 與 `token_removed` 兩列的 `msg` 就是它，逐字相同（本票實測）。
 *   舊釘法只涵蓋那一句；新釘法涵蓋後端送出來的**每一句**，所以把退路放回去（無論它比對
 *   的是哪一句）都會紅，而且紅字會指名是哪一列。
 * ⬛ 而且它仍然**一個中文句子都不手寫**：句子全部來自後端產的副本，後端改文案 ⇒ 副本重產
 *   ⇒ 這一條跟著吃新的句子。
 * ══════════════════════════════════════════════════════════════════════ */
const 墓碑 = [
  '🪦 這條退路在 2026-09-18 被拆掉（liff #74，擁有者知情拍板）。**它不該在了。**',
  '',
  '⬛ 拆的那天量到的（後端 main e2ec970，本票獨立重跑）：',
  '   尺：不帶代號=0 具名豁免=3 帶代號=111 不是權限拒絕=319 全部=433',
  '   行為：後端守門 `allow === null` 那三格 revoked → ok（三格全變）；契約表 17 列 0 列改變。',
  '   真瀏覽器：種 7 把加密快取 → 餵那三格的信封 → 拆前「0 把＋整頁覆蓋層」、',
  '            拆後「7 把仍在＋無覆蓋層」；真撤權（role_unresolved）拆前拆後都是',
  '            「0 把＋整頁覆蓋層」⇒ 沒有少遮蔽任何一條真的撤權路徑。',
  '',
  '🔴 **這一條紅了，代表「靠 msg 文字判 revoked」被放回來了。** 放回去＝同時放回',
  '   「權限好好的人打到一個不存在的 action 名 ⇒ 那一頁對他當場報廢」。要放回去之前先讀',
  '   assets/board-cache.js 的 cacheVerdict 檔頭 🪦 那一段，那裡有完整推導與對照組。',
  '',
  '⚠️ 另一種讓這一條紅的可能：後端某一句拒絕文案改成以「連線逾時」／「連線失敗」開頭',
  '   ⇒ 它會被**離線**那條前綴比對接走、回 offline。那不是本條在防的缺陷，',
  '   但它本身是真問題（權限拒絕被當成離線）⇒ 先確認是哪一種再動手。',
].join('\n');

test('🪦 抽掉代號的拒絕一律 ok——不准再靠 msg 文字判 revoked（liff #74／gas #203）', () => {
  // 🔴 **句子全部從後端讀，不手寫**。母體＝契約表裡每一列真的被拒絕的信封。
  const 拒絕 = 契約.denials.filter((r) => r.envelope && r.envelope.ok === false
    && String(r.envelope.msg || '') !== '');
  assert.ok(拒絕.length >= 10,
    '契約表裡只有 ' + 拒絕.length + ' 列帶文案的拒絕 ⇒ 下面那一圈沒有母體，等於什麼都沒測');

  // ⬛ 對照組①（**最重要的一格**）：代號那條路仍然在作用。
  //    沒有它，下面「全部回 ok」也可能只是因為 `cacheVerdict` 恆回 ok——那才是真正
  //    危險的假綠燈（退路拆了、連撤銷遮蔽也一起壞了，而這一條照樣綠）。
  const 帶代號會撤權 = 拒絕.filter((r) => BC.cacheVerdict(r.envelope) === 'revoked');
  assert.ok(帶代號會撤權.length >= 2,
    '契約表裡只有 ' + 帶代號會撤權.length + ' 列「帶代號會判 revoked」⇒ 撤銷遮蔽壞了，'
    + '而下面那一圈「抽掉代號回 ok」在這種狀態下是恆真的');

  // 🔴 本體：同樣那幾句話，**抽掉代號**之後一律 ok。
  拒絕.forEach((r) => {
    assert.equal(BC.cacheVerdict({ ok: false, msg: r.envelope.msg }), 'ok',
      '\n\n' + 墓碑
      + '\n  出事的那一句來自契約表的 `' + r.key + '`：' + JSON.stringify(r.envelope.msg) + '\n');
  });

  // ⬛ 對照組②：離線那條路仍在、成功仍是 ok（沿用原本那幾格）。
  assert.equal(BC.cacheVerdict({ ok: false, msg: '連線失敗' }), 'offline',
    '⚠️ 離線那條路也一起被拆掉了 ⇒ 上面那一圈的 ok 是「什麼都判 ok」的假綠燈');
  assert.equal(BC.cacheVerdict({ ok: true }), 'ok');
  // 🔴 這條退場條件的**另一個端點在後端**（`gateContract.legacyDenyMsg` 那一格匯出）。
  //    本條已經不讀它 ⇒ 後端拆掉之後，本條仍然成立（gas `#203` 的順序：liff 先、gas 後）。
});
