...
const 墓碑 = [
  ...
  '',
  '🔴 **這一條紅了，代表那條退路被放回去了。** 放回去＝同時放回「權限好好的人打到一個',
  '   不存在的 action 名 ⇒ 那一頁對他當場報廢」。要放回去之前先讀',
  '   assets/board-cache.js 的 cacheVerdict 檔頭 🪦 那一段，那裡有完整推導與對照組。',
  '',
  '⚠️ 另一種讓這一條紅的可能：後端把 legacyDenyMsg 那一格匯出拿掉、或改了那一句。',
  '   那不是缺陷 ⇒ 先確認是哪一種再動手。',
].join('\n');

test('🪦 不帶代號的拒絕一律 ok——那條 msg 前綴退路不該再回來（liff #74）', () => {
  // 🔴 **那一句從後端讀**（`gateContract.legacyDenyMsg`），不手寫。
  ...
  assert.equal(BC.cacheVerdict({ ok: false, msg: 退路那句 }), 'ok', '\n\n' + 墓碑 + '\n');
  // ⬛ 對照組：沒有這幾格，上面那句「回 ok」也可能只是因為 `cacheVerdict` 恆回 ok。
  //    這幾格分別釘住：代號那條路仍在作用／離線那條路仍在作用／成功仍是 ok。
  assert.equal(BC.cacheVerdict({ ok: false, msg: 退路那句, reason: 'role_unresolved' }), 'revoked',
    '⚠️ 同一句話帶上代號竟然不是 revoked ⇒ 這次拆的不只是那條退路，撤銷遮蔽壞了');
  assert.equal(BC.cacheVerdict({ ok: false, msg: '連線失敗' }), 'offline',
    '⚠️ 離線那條路也一起被拆掉了 ⇒ 上面那個 ok 是「什麼都判 ok」的假綠燈');
  assert.equal(BC.cacheVerdict({ ok: false, msg: '找不到員工名冊' }), 'ok');
  assert.equal(BC.cacheVerdict({ ok: true }), 'ok');
  // 🔴 這條退場條件有**兩個端點**：本檔這條退路（已拆），與後端那一格把 `legacyDenyMsg`
  //    交出來的匯出（**還在**，跨 repo、另開票）。⚠️ 後端要拿掉它之前先改這一條，
  //    否則這一條會因為副本缺欄位而紅，而紅的理由跟真正的缺陷無關。
});
