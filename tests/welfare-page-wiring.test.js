 ...
 *    **那是它在做事，不是它壞了。**
 */
/* ══ 2026-09-14：三處不再是同一個名字 ════════════════════════════════════
 *
 * SPEC `jdc-tw/jdc-line-gas#13` 第②項拍板：這一頁改名為 `line.html`、
 * 標題「line訊息發訊」、**加進分流表**（舊檔名逐字寫在 `line.html` 檔頭的
 * 「原網址」那一行，本檔刻意不再複製一份）。
 *
 * 🔴 **被改的是「分頁標題」這一處，不是頁面的顯示名稱。** 依據不是推論：
 *    同一份 SPEC 的分流頁表把六頁列成「活動紀錄看板／活動出席狀況／LINE 訊息紀錄／
 *    line訊息發訊／人事異動看板／人事資訊看板」，而前五者逐字等於各該頁的 `<title>`
 *    （stats／attend／messages／board／hr-stats）。⇒ `line訊息發訊` 是**分流表這一格的
 *    名字＝`<title>`**，2026-09-08 定的顯示名稱「日本國土開發官方 LINE 傳送平台」
 *    沒有被這項決定碰到，畫面上的 `<h1>` 與身分閘標題維持原樣。
 *
 * ⚠️ 這不是把 2026-09-08 那條判準放寬去湊綠燈——**兩個方向都還在**：三處仍然逐處
 *    被斷言、舊名稱仍然只准出現在標「原名」的那一行。改的只是「第一處該等於哪個字串」。
 *    要驗這條還咬得住，把 `<h1>` 也改成 `line訊息發訊` 看它會不會紅。
 *
 * ⚠️ **如果使用者的原意是整頁改名**（`<h1>` 與身分閘也要變成「line訊息發訊」），
 *    那就把 DISPLAY_NAME 也改成 TAB_NAME 的值、並把 line.html 那兩處一起改，
 *    **不要只改這支測試**——只改測試會讓畫面與斷言一起說謊。
 */
const TAB_NAME = 'line訊息發訊';                      // 分頁標題＝分流表那一格的名字
const DISPLAY_NAME = '日本國土開發官方 LINE 傳送平台';  // 畫面上的顯示名稱（2026-09-08 拍板）
...
test('頁面名稱是新的：分頁標題、身分閘標題、主標題三處都要', () => {
  ...
  assert.deepEqual(少的, [],
    ...
    + `——分頁標題要是「${TAB_NAME}」（分流表那一格），`
    + `畫面上的兩處要是「${DISPLAY_NAME}」；三處各自都要對。`);
});
...
test('⬛ 對照組：倒數還沒歸零時，講的是「還有幾秒」（否則上一條恆真）', () => {
  ...
  // 🔴 釘的是**字面值**，不是「有沒有提到取消」——後者會 fail-open。
  //    這一句的內容（關掉頁面不會取消）是規格，改字就該讓這條紅一次、由人確認。
  //    2026-09-14：文案從「還有 N 秒可以取消（關掉這一頁不會取消）」縮成
  //    「還有 N 秒（關掉頁面不會取消）」——旁邊的按鈕已經寫著「取消這次發送」，
